...
class BilibiliWatcher(BilibiliClientBase):
    ...
    async def watch(self, url: str) -> Dict[str, Any]:
        ...
        """
        direct_bvid = self._resolve_bvid(url=url)
        if direct_bvid:
            return await self._watch_bilibili(direct_bvid, url)
    ...
    async def _watch_bilibili(self, bvid: Optional[str], url: str) -> Dict[str, Any]:
        ...
        """
        if not bvid:
            bvid = self._resolve_bvid(url=url)
        if not bvid:
            return {"success": False, "message": f"Cannot extract BV ID from: {url}"}

        try:
            detail = await self._fetch_video_detail_data(bvid)
        except Exception as exc:
            return self._failure(str(exc), url=url, bvid=bvid)

        video = detail.get("View", {})
        summary = self._normalize_video_summary(video)
        tags = [t.get("tag_name") for t in detail.get("Tags", []) if t.get("tag_name")]
        related = []
        for r in detail.get("Related", [])[:5]:
            related.append({
                "bvid": r.get("bvid"),
                "title": r.get("title"),
    ...
    async def get_stats(self, url: str) -> Dict[str, Any]:
        ...
        try:
            video = await self._fetch_video_info_data(bvid)
        except Exception as exc:
            return self._failure(str(exc), url=url, bvid=bvid)

        stats = self._normalize_video_summary(video).get("stats", {})
        return {
            "success": True,
            "bvid": bvid,
