...
class SubtitleDownloader:
    ...
    async def _fallback_get_text(
        ...
        Preferred fallback: local OpenAI-compatible STT API using the local
        Qwen3 ASR model path. Secondary fallback: danmaku as a rough text
        reference. Whisper-based fallback was intentionally removed because it
        produced misleading/hallucinated subtitles.
        ...
        Returns:
            Combined result dict with local STT and/or danmaku fallback.
        """
        results = {
            "success": False,
            "bvid": bvid,
            "title": title,
            "cc_subtitle": False,
            "message": "No CC subtitles available. Attempting local STT fallback...",
        }

        transcribe_result = await self._local_stt_fallback(url, bvid, title, format, output_dir)
        if transcribe_result.get("success"):
            results["success"] = True
            results["transcription"] = transcribe_result
        else:
            results["transcription"] = {"success": False, "message": transcribe_result.get("message", "Local STT failed")}
        ...
        if results.get("transcription", {}).get("success") and results.get("danmaku", {}).get("success"):
            results["message"] = "No CC subtitles found. Local STT and danmaku fallback provided."
        elif results.get("transcription", {}).get("success"):
            results["message"] = "No CC subtitles found. Local STT fallback provided."
        elif results.get("danmaku", {}).get("success"):
            results["message"] = "No CC subtitles found. Danmaku fallback provided after local STT failed."
        else:
            results["message"] = "No CC subtitles found. Local STT and danmaku fallback both failed."

        return results

    async def _local_stt_fallback(
        self,
        url: str,
        bvid: str,
        title: str,
        ...
        """Fallback: download audio and transcribe it with the local STT API.

        Uses the local OpenAI-compatible STT endpoint and the explicit model
        path because that path has proven more reliable than alias-based model
        IDs in this environment.
        """
        if not self.downloader:
            return {
                "success": False,
                "message": "Local STT fallback requires a downloader instance.",
            }

        stt_base_url = os.environ.get("BILIBILI_LOCAL_STT_BASE_URL", "http://127.0.0.1:8000/v1")
        stt_api_key = os.environ.get("BILIBILI_LOCAL_STT_API_KEY", "local-noauth")
        stt_model = os.environ.get("BILIBILI_LOCAL_STT_MODEL", "./models/Qwen3-ASR-0.6B-bf16")
        stt_language = (os.environ.get("BILIBILI_LOCAL_STT_LANGUAGE") or "").strip()
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            wav_path = os.path.join(tmp_dir, f"{sanitize_filename(title or bvid)}.wav")
            try:
                subprocess.run(
                ...
                return {
                    "success": False,
                    "message": f"Audio conversion to wav failed: {exc}",
                }
            ...
            try:
                with open(wav_path, "rb") as audio_file:
                    ...
                    async with httpx.AsyncClient(timeout=120.0) as client:
                        resp = await client.post(
                            f"{stt_base_url.rstrip('/')}/audio/transcriptions",
                ...
                return {
                    "success": False,
                    "message": f"Local STT request failed: {exc}",
                }
        ...
        if not text:
            return {
                "success": False,
                "message": "Local STT returned no transcript text.",
            }

        segments = payload.get("segments") or [{"start": 0.0, "end": 0.0, "text": text}]
        body = []
        for idx, segment in enumerate(segments):
            ...
            end = float(segment.get("end", start + 3.0) or (start + 3.0))
            if end <= start:
                end = start + 3.0
            body.append({
                "from": start,
        ...
        if not body:
            body = [{"from": 0.0, "to": max(3.0, len(text) / 8), "content": text}]

        safe_title = sanitize_filename(title or bvid)
        filename = f"{safe_title}_local_stt.{format}"
